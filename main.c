...
void initialiserMemoireSecondaire(FILE* ms) {
    ...
    sprintf(buffer.enregs[0].nom, "Bloc_%d", 0);
    ...
    strcpy(buffer.metadata.fichier_nom, "table_allocation");
    ...
    if (fwrite(&buffer, sizeof(buffer), 1, ms) != 1) {
        printf("Erreur : Impossible d'écrire dans 'memoire_secondaire.dat'.\n");
    } else {
...
void chargerContiguOrdonne(FILE *ms, METADATA *metadata) { // Implémentation de la fonction
  ...
  void chargerContiguNonOrdonne(FILE *ms, METADATA *metadata) { // Implémentation de la fonction
  } 
...
void insertionContigueNonOrdonnee(FILE *ms, const char *nom_fichier, ENREG nouvel_enreg) {
    printf("Insertion contiguë non ordonnée\n");
    printf("Nom du fichier : %s\n", nom_fichier);
    
    
    // Logique d'insertion ici...
    
    printf("Insertion terminée dans l'ordre contigu non ordonné.\n");
}
...
// Suppression logique : marquer comme supprimé dans une organisation contiguë et non ordonnée
void suppressionContigueNonOrdonneeLogique(FILE *ms, const char *nom_fichier, int id) {
    printf("Suppression logique de l'enregistrement avec ID %d dans une organisation Contiguë et NONORDONNE.\n", id);
    // Implémenter la logique pour marquer l'enregistrement comme supprimé (par exemple, changer un flag ou une valeur spéciale)
}

// Suppression physique : réorganiser les blocs pour libérer l'espace dans une organisation contiguë et non ordonnée
void suppressionContigueNonOrdonneePhysique(FILE *ms, const char *nom_fichier, int id) {
    printf("Suppression physique de l'enregistrement avec ID %d dans une organisation Contiguë et NONORDONNE.\n", id);
    // Implémenter la logique pour supprimer physiquement l'enregistrement et réorganiser les blocs (déplacer, retirer l'enregistrement du bloc, etc.)
}
...
// Fonction pour défragmenter un fichier dans une organisation contiguë et non ordonnée
void defragmentationContigueNonOrdonnee(FILE *ms, const char *nom_fichier) {
    printf("Défragmentation dans une organisation Contiguë et NONORDONNE.\n");
    // Logique pour déplacer les enregistrements et réorganiser les blocs
    // Cela pourrait impliquer de déplacer les enregistrements dans les blocs disponibles
}
