...
void chargerContiguOrdonne(FILE *ms, METADATA *metadata) {
    ...
    printf("Chargement des blocs contigus ordonnés effectué avec succès.\n");
}
...
POSITION rechercherContigueOrdonnee(FILE *ms, const char *nom_fichier, int id) {
    ...
    for (int i = 0; i < taille_fichier; i++) {
        ...
        for (int j = 0; j < buffer.enregs_utilises; j++) {
            ...
            if (buffer.enregs[j].id > id) { // Si l'ID est supérieur, on peut arrêter la recherche
                break; // Sortir de la boucle, car les enregistrements sont ordonnés
            }
        }
    }
...
void insertionContigueOrdonnee(FILE *ms, const char *nom_fichier, ENREG nouvel_enreg) {
    ...
    // Vérifier si l'ID est unique en appelant rechercherContigueOrdonnee
    ...
    for (int i = 0; i < taille_fichier; i++) {
        ...
        if (buffer.enregs_utilises < FACTEUR_BLOCS) {
            // Insérer le nouvel enregistrement à la bonne position pour maintenir l'ordre
            int j;
            ...
            buffer.enregs[j] = nouvel_enreg;  // Insérer le nouvel enregistrement
            buffer.enregs_utilises++;
            fseek(ms, -sizeof(BLOC), SEEK_CUR);  // Revenir en arrière pour réécrire le bloc
    ...
    nouveau_bloc.enregs[0] = nouvel_enreg;  // Insérer le nouvel enregistrement
...
void defragmentationContigueOrdonnee(FILE *ms, const char *nom_fichier) {
 ...
 if (fread(&buffer, sizeof(BLOC), 1, ms) != 1) {
        ...
        // Si le bloc actuel devient vide, mettre à jour la table d'allocation
